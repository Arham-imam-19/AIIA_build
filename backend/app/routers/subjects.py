...
class VisitQueryUpdate(BaseModel):
    model_config = ConfigDict(extra="forbid")
    query_text: str

@router.patch("/visits/{visit_id}/query", response_model=Visit)
def raise_visit_query(
    visit_id: int,
    body: VisitQueryUpdate,
    session: Session = Depends(get_session),
    user: CurrentUser = Depends(require(Permission.VISIT_WRITE)),
) -> Visit:
    visit = session.get(Visit, visit_id)
    ...
    old_value = json.dumps({"has_query": visit.has_query, "query_text": visit.query_text}, sort_keys=True)
    visit.has_query = True
    visit.query_text = body.query_text
    visit.updated_at = utcnow()
    session.add(visit)
    
    new_value = json.dumps({"has_query": True, "query_text": visit.query_text}, sort_keys=True)
    audit.record(
        session,
        user=user,
        action=AuditAction.UPDATE,
        ...
        old_value=old_value,
        new_value=new_value,
        reason=f"Query raised: {body.query_text}",
        trial_id=subject.trial_id,
    )
    session.commit()
    session.refresh(visit)
    from app.events import bus
    bus.broadcast(str(user.id), "dashboard_updated", {"message": f"Query raised on {visit.visit_name}"})
    return visit
